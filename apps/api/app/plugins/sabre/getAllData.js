...
} = require('./services/serviceBFM')
const { serviceGetTokenSabre } = require('./services/servicesAuth')

const getAllData = (params, destructureData = true) =>
      ...
      utils.httpRequest$(url, 'post', headers, body).subscribe(
        async (res) => {
          if (destructureData) {
            ...
            response.data = await serviceTransformFlightData(res).catch(() => {
              reject('PROBLEM_LOAD_FLIGHTS')
            })
            resolve(response)
          } else {
