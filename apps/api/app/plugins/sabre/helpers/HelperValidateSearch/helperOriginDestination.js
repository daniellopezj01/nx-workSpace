  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < schedules.length; i++) {
        ...
        for (
          let indexInternal = 0;
          indexInternal < itemSchedule.length;
          indexInternal++
        ) {
          const singleFlight = itemSchedule[indexInternal]
          const classOfService =
            currentSegment.segments[indexInternal].segment.bookingCode
          const flight = await helperStructureFlight(
            singleFlight,
            currentDate,
            classOfService
          )
