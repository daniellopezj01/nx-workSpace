...
const helperPnrAirPrice = (passengers = [], { price = {} }) => new Promise(async (resolve, reject) => {
  const arrayAirPrice = []
  if (passengers.length) {
    ...
    const totalPrice = numStr.includes('.') ? parseFloat(numStr.replace('.', '')) : price?.netPrice
    const setting = await db.findOneBoolean({ key: 'sabreCurrency' }, settingsModel)
    const newPrice = setting ? Number(parseInt(price?.netPrice / setting.value.exchangeRateUsed, 10)) : totalPrice
    arrayAirPrice[0] = {
      PriceComparison: {
