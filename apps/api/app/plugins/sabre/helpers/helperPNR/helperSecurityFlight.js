...
const helperSecurityFlight = (passengers, dataAirline) => new Promise(async (resolve) => {
  const secureFlight = []
  const { includedMainAirline, listAirline } = dataAirline
  const onlyDefaultAirline = _.every(listAirline, (i) => i === process.env.MAIN_AIRLINE)
  _.map(passengers, async (i, index) => {
    const {
      gender, dateOfBirth, name, surname
    } = i
