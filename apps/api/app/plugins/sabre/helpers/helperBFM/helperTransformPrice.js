...
const helperTransformPrice = (pricingInformation = {}, transformPrice) => new Promise(async (resolve, reject) => {
  try {
    ...
    let price = totalFare
    const operationalCharges = Number(Math.trunc(price?.totalPrice * process.env.COMISSION_SABRE) / 100, 2)
