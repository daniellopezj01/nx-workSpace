...
const { serviceRequestValidateSearch } = require('./services/servicesValidateSearch')
const db = require('../../middleware/db')
const { helperIncludedMainAirline } = require('./helpers/HelperValidateSearch')

const validateSearch = (params) => new Promise(async (resolve, reject) => {
  try {
    const { adults, children } = params
    let objectData = await serviceRequestValidateSearch(params).catch((err) => {
      utils.buildErrObjectReject(err, reject, '422', 'REQUEST_ERROR_VALIDATE_SEARCH')
    })
    ...
    objectData = {
      ...objectData,
      schedules,
      originalDestination: OTA_AirLowFareSearchRQ.OriginDestinationInformation,
      codeItinerary,
    ...
    utils.buildErrObjectReject(error, reject, '422', 'CATCH_ERROR_VALIDATE_SEARCH')
  }
})
