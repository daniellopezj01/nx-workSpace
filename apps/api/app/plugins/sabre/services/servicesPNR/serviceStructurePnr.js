...
const serviceStructurePnr = (flightOrder) => new Promise(async (resolve, reject) => {
  try {
    ...
    const infoAirPrice = await helperPnrAirPrice(passengers, flightOrder.data[0])
    const objectRequestDetails = await helperSpecialRequestDetails(flightOrder)
    const newParams = {
