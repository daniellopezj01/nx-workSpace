...
const { serviceTransformFlightData } = require('../serviceBFM/serviceTransformFlightData')

const serviceRequestValidateSearch = (params, changeParams = true) => new Promise(async (resolve, reject) => {
  try {
    const token = await serviceGetTokenSabre()
    const headers = await utils.structureTokenBearer(token)
    const body = changeParams ? await serviceStructureParamsValidateSearch(params) : params
    ...
    utils.httpRequest$(url, 'post', headers, body).subscribe(
      async (res) => {
        ...
        response.data = await serviceTransformFlightData(res, true)
          .catch((e) => {
            console.log(e)
      ...
      (error) => {
        ...
        utils.buildErrObjectReject(error, reject, '422', 'REQUEST_VALIDATE_SEARCH')
      }
    )
  } catch (error) {
    utils.buildErrObjectReject(error, reject, '422', 'REQUEST_VALIDATE_SEARCH')
