...
const serviceTransformFlightData = (
  { groupedItineraryResponse },
  transformPrice = false
) =>
  new Promise(async (resolve, reject) => {
    try {
      const { scheduleDescs, legDescs, itineraryGroups, messages } =
        groupedItineraryResponse
      ...
      if (globalItineraries) {
        ...
        for (let j = 0; j < itineraries.length; j++) {
          ...
          const priceData = await helperTransformPrice(
            pricingInformation,
            transformPrice
          )
          ...
          item.segments =
            _.head(
              pricingInformation
            ).fare.passengerInfoList[0].passengerInfo.fareComponents
