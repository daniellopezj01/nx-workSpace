...
const generatePnr = (locale, { params }, user) =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      utils.httpRequest$(url, 'post', headers, paramsPnr).subscribe(
        async (res) => {
          const { ApplicationResults, ItineraryRef: idPnr } =
            res.CreatePassengerNameRecordRS
          ...
          const data = await db
            .updateItem(flight._id, flightModel, object)
            .catch((err) => {
              console.log(err)
            })
