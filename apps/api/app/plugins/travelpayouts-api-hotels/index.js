...
const { tap, delay, retryWhen, switchMap } = require('rxjs/operators')
...
const destructuringParamsForUrl = (params) =>
  new Promise((resolve) => {
    let paramUrl = ''
...
const checkParamsHotelsByNameOrLocation = async (params) =>
  new Promise((resolve, reject) => {
    try {
      const { query, lang, lookFor, limit, convertCase } = params
      resolve({
...
const checkParamsCostOfLivinInHotels = async (params) =>
  new Promise((resolve, reject) => {
    try {
      const {
        location,
...
const checkParamsHotelsSelections = async (params) =>
  new Promise((resolve, reject) => {
    try {
      const { check_in, check_out, currency, language, limit, type, id } =
        params
      resolve({
...
const checkParamsTypesHotelCollections = async (params) =>
  new Promise((resolve, reject) => {
    try {
      const { id } = params
      resolve({
...
const checkedSearchManagement = async (params) =>
  new Promise((resolve, reject) => {
    try {
      const {
        cityId,
...
const createSignature = async (query) =>
  new Promise((resolve) => {
    const organized = {}
    query.marker = null
    Object.keys(query)
...
const secondRequest = (searchId, params) => {
  let { sortBy, sortAsc, limit, offset, roomsCount } = params
  sortBy = sortBy || null
  sortAsc = sortAsc || '0'
  limit = limit || 0
  offset = offset || 0
 ...
 */
exports.get_cost_in_hotels = async (params) =>
  new Promise(async (resolve, reject) => {
    try {
...
exports.get_hotels_selections = async (params) =>
  new Promise(async (resolve, reject) => {
    try {
 ...
 */
exports.get_types_hotel_collections = async (params) =>
  new Promise(async (resolve, reject) => {
    try {
      const query = await checkParamsTypesHotelCollections(params)
      const root =
        'http://yasen.hotellook.com/tp/public/available_selections.json?'
      const queryParams = await destructuringParamsForUrl(query)
      const url = `${root}${queryParams}`
      const data = await utils.httpRequest(url, 'get')
      resolve(data)
 ...
 * @param {*} params
 */
exports.get_search_management = (params) =>
  new Promise(async (resolve, reject) => {
    try {
      // console.log(params)
      ...
      const finalHttp$ = utils.httpRequest$(url, 'get', {}, {}).pipe(
        ...
        switchMap((resultFirst) =>
          utils
            .httpRequest$(
              secondRequest(resultFirst.searchId, params),
              'get',
              {},
              {}
            )
...
exports.get_hotel_single = (params) =>
  new Promise(async (resolve, reject) => {
    try {
