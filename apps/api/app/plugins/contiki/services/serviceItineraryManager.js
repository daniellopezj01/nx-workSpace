...
const serviceItineraryManager = ({ itinerary }, tour) => new Promise(async (resolve, reject) => {
  try {
    _.map(itinerary, (i) => (i.locationsVisited = _.head(i.locationsVisited)))
    const newItineraries = await helpersItineraryNights(itinerary).catch((e) => { console.log('noches', e.message) })
    for (let i = 0; i < newItineraries.length; i++) {
      ...
      const {
        idTour, idTourExternal, idOptionTour, itineraryName
      } = newItinerary
      const filter = {
        idTour, idTourExternal, idOptionTour, itineraryName
      }
      ...
      if (checkItinerary) {
        await db.updateItem(checkItinerary._id, itineraryModel, newItinerary)
          .catch((err) => { console.log('error update itinerary', err.message) })
      } else {
        await db.createItem(newItinerary, itineraryModel)
          .catch((err) => { console.log('error create itinerary', err.message) })
      }
    }
