...
const serviceDepartureManager = (seasons, tour) =>
  new Promise(async (resolve, reject) => {
    try {
      let departures = []
      _.map(seasons, (i) => {
        departures = _.concat(departures, i.departures)
      })
      ...
      for (let i = 0; i < departures.length; i++) {
        ...
        const { idTour, idTourExternal, idOptionTour, idExternal } = departure
        const filter = {
          idTour,
          idTourExternal,
          idOptionTour,
        ...
        if (checkDeparture) {
          await db
            .updateItem(checkDeparture._id, departureModel, departure)
            .then(({ _id: idDeparture }) => {
              ids.push(`${idDeparture}`)
            })
            .catch((err) => {
              console.log('error update departure', err.message)
            })
        } else {
          await db
            .createItem(departure, departureModel)
            .then(({ _id: idDeparture }) => {
              ids.push(`${idDeparture}`)
            })
            .catch((err) => {
              console.log('error create departure', err.message)
            })
        }
      }
      resolve(ids)
