...
const serviceTourStructure = (general, details, continents, agency) =>
  new Promise(async (resolve, reject) => {
    const { id, name, brand, content } = general
    ...
    if (!data?.images) {
      ...
      const checkTour = await db.findOneBoolean(
        { idOptionTour: details.id },
        tourModel
      )
      if (checkTour) {
        delete tour.slug
        tour = await db
          .updateItem(checkTour._id, tourModel, tour)
          .catch((err) => {
            console.log('error update', err.message)
          })
      } else {
        tour = await db.createItem(tour, tourModel).catch((err) => {
          console.log('error create ', err.message)
        })
      }
      await serviceItineraryManager(data, tour).catch((e) => {
        console.log('itinerary', e.message)
      })
      await serviceDepartureManager(seasons, tour).catch((e) => {
        console.log('departure', e.message)
      })
      resolve(tour)
    }
  })
