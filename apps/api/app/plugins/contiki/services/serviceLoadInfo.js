...
const serviceLoadInfo = (data) => new Promise(async (resolve, reject) => {
  try {
    let continents = await db.getItems({ query: { limit: 15 } }, modelContinents, {})
    continents = continents.docs
    const agency = await db.findOne({ email: process.env.DEFAULT_EMAIL_AGENCY }, modelUser)
    if (data.length) {
