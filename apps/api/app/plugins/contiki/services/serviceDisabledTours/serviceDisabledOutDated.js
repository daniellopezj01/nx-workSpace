...
const serviceDisabledOutDated = (currentToursContiki = []) => new Promise(async (resolve, reject) => {
  try {
    const allOwnTours = await db.find({ idExternal: { $exists: true } }, modelTour, 'idExternal idOptionTour status')
    ...
    const toDisable = allOwnTours.filter(
      (i) => !_.find(currentToursContiki, (o) => i.idExternal === o.idExternal && i.idOptionTour === o.idOptionTour)
    )
    ...
    if (toDisable.length) {
      for (let index = 0; index < toDisable.length; index++) {
        ...
        if (status === 'publish') {
          await db.updateItem(`${_id}`, modelTour, { status: 'construction' }).then(() => {
