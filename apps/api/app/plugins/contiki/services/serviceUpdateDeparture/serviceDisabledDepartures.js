...
const serviceDisabledDepartures = (idTour, idDepartures = []) =>
  new Promise(async (resolve, reject) => {
    try {
      const allDepartures = await db.find(
        { idTour },
        modelDeparture,
        '_id status'
      )
      ...
      if (forDisabled.length) {
        for (let index = 0; index < forDisabled.length; index++) {
          ...
          if (status === 'visible') {
            await db
              .updateItem(`${_id}`, modelDeparture, { status: 'not_visible' })
              .then((res) => {
      ...
      const activeDepartures = await db.find(
        { idTour, status: 'visible' },
        modelDeparture,
        '_id status'
      )
      if (activeDepartures.length === 0) {
        await db
          .updateItem(idTour, modelTour, { status: 'construction' })
          .then(() => {
            console.log('update empty  departures tour', idTour)
          })
          .catch(() => {
            console.log('error update empty  departures tour', idTour)
          })
      }
      resolve({})
    } catch (error) {
