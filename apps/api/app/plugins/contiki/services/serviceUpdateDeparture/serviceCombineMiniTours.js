...
const serviceCombineMiniTours = (general, tourList) =>
  new Promise(async (resolve, reject) => {
    try {
      tourList = _.chain(tourList)
        .uniqBy((i) => _.head(i.websiteUrls).url)
        .filter((o) => _.head(o.websiteUrls).url)
        .value()
      if (tourList.length) {
        ...
        for (let i = 0; i < tourList.length; i++) {
          ...
          console.log(
            `******** idTour ${general.id}  --- mini tours: ${tourDetails.id}`
          )
          const { seasons, id: idOptionTour } = tourDetails
          const { id: idExternal } = general
          await db
            .findOneBoolean({ idExternal, idOptionTour }, tourModel)
            .then(async (tour) => {
              const isdDepartures = await serviceDepartureManager(
                seasons,
                tour
              ).catch((e) => {
