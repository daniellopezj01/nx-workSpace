  ...
  return new Promise((resolve, reject) => {
    fs.readFile(
      ...
      (err, data = {}) => {
        ...
        const close = moment(departure?.closeDateDeparture, 'DD-MM-YYYY').format('DD [de] MMMM YYYY')
        data = data.replace(/TITLE_TOUR/g, tour?.title)
        data = data.replace(/NAME_USER/g, `${reservation?.travelerFirstName} ${reservation?.travelerLastName || ''}`)
