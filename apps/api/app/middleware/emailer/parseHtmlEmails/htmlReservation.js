  ...
  return new Promise((resolve, reject) => {
    fs.readFile(
      ...
      (err, data = {}) => {
        ...
        const begin = moment(departure?.startDateDeparture, 'DD-MM-YYYY').format('DD [de] MMMM YYYY')
        const end = moment(departure?.endDateDeparture, 'DD-MM-YYYY').format('DD [de] MMMM YYYY')
        ...
        data = data.replace(/TRAVELER_NAME/g, `${object?.travelerFirstName} ${object?.travelerLastName || ''}`)
        data = data.replace(/BEGIN_TOUR/g, begin)
        data = data.replace(/END_TOUR/g, end)
        data = data.replace(
          /PRICE/g,
          parseFloat(object.amount).toFixed(2)
        )
        resolve(data)
      }
    )
